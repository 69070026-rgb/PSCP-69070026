...
ห้องที่อยู่ N
Output:
จำนวนกำแพงน้อยที่สุดที่จะต้องพัง
Constraints:
ถ้าไม่ต้องพังกำแพงเลย ให้ตอบ 0
```
...
```text
1. รับค่า N และกำหนดค่าห้อง L= 1
2 loop โดยที่  L*L < N
3.ให้ L += 1
4.กำหนดห้องสะสม  p = N - (L - 1) ** 2
5.ถ้า p เป็นเลขคี่ ให้ print(2 * (L - 1))
6. ถ้าp เป็นเลขคู่ ให้ print(2 * L - 3)
```
